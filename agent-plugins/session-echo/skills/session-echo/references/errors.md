...
Run `anarlog --json doctor`. Ask the user to open Session Echo once if the database does not exist. If they keep data in a custom location, use `--db-path FILE` or `ANARLOG_DB_PATH` after they provide the path.
