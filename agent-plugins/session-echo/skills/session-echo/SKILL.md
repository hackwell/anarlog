---
name: session-echo
description: Query Session Echo meetings, notes, summaries, transcripts, participants, action items, and recurring history. Use when a user asks about their Session Echo meeting data or needs meeting context for another task.
---

# Session Echo

Use Session Echo's local interfaces. Prefer MCP when its tools are connected. Otherwise use the `anarlog` CLI with `--json`. Meeting reads are safe. Writes are limited to staging proposals.
...
3. If neither is available, ask the user to install Session Echo. Do not install software unless the user asks.

Never query or modify Session Echo's SQLite database directly. The CLI and MCP server handle application-schema compatibility.
...
- Do not claim to update meetings. CLI and MCP can only stage a proposal. A human applies or declines it in the Session Echo desktop app.
