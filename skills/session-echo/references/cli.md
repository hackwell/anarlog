...
`proposals create` stages a pending edit. Do not claim the meeting changed. A human applies or declines it in the Session Echo desktop app. Provide either `--content` for inline markdown or `--content-file` to read it from a file; exactly one is required. For summaries, the `--target` option names which existing summary to replace (ignored for memos); it is required when a meeting has multiple summaries, and optional when exactly one exists.
