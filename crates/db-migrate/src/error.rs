...
pub enum MigrateError {
    ...
    // "created by a newer version of Session Echo" in the rendered message.
    #[error(
        "the database was created by a newer version of Session Echo: it requires migration {min_supported_version}, but this build only includes migrations up to {max_known_version}"
    )]
    SchemaFromNewerApp {
    ...
    // "created by a newer version of Session Echo" in the rendered message.
    #[error(
        "migration {missing_version} cannot be applied because the database was created by a newer version of Session Echo (max applied migration {max_applied_version})"
    )]
    DatabaseAhead {
        missing_version: i64,
