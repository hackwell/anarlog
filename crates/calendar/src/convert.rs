...
fn convert_apple_event(event: AppleEvent) -> CalendarEvent {
    ...
    let meeting_link = resolve_meeting_link(event.location.as_deref(), event.notes.as_deref());
...
// A link parsed from the location wins over one parsed from the description, so
// every event crosses the Tauri bridge with its final meeting link resolved.
fn resolve_meeting_link(location: Option<&str>, description: Option<&str>) -> Option<String> {
    location
...
mod meeting_link_tests {
    ...
    fn location_link_wins_over_description_link() {
        assert_eq!(
            resolve_meeting_link(Some(MEET_LINK), Some(CAL_LINK)),
    ...
    fn description_link_is_the_last_fallback() {
        assert_eq!(
            resolve_meeting_link(Some("Conference room 4"), Some(CAL_LINK)),
    ...
    fn no_link_stays_absent() {
        assert_eq!(
            resolve_meeting_link(Some("Conference room 4"), Some("Agenda")),
            None
        );
