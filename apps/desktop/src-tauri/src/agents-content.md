# Session Echo Desktop

This file is auto-generated on app startup.

## Meeting data

Use Session Echo's typed, read-only interfaces for meeting data. Do not use `find`,
`grep`, `rg`, filesystem crawling, or direct SQLite queries to find or read
meetings.

Prefer the Session Echo MCP tools when they are available:
...
If MCP is unavailable, use the Session Echo CLI with `--json`:
...
The CLI discovers Session Echo's database from the platform application-data
