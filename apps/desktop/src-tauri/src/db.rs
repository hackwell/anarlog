...
pub fn is_newer_schema_error(error: &impl std::fmt::Display) -> bool {
    error
        .to_string()
        .contains("created by a newer version of Session Echo")
}
...
mod tests {
    ...
    fn newer_schema_errors_are_recognized() {
        ...
        assert!(is_newer_schema_error(
            &"plugin db failed: the database was created by a newer version of Session Echo: it requires migration 20260901000000, but this build only includes migrations up to 20260816100100"
        ));
        assert!(!is_newer_schema_error(
            &"unable to open database file: /tmp/app.db"
        ));
    ...
    fn dev_uses_an_isolated_persistent_database() {
        let db_dir = desktop_db_dir("de.flagbit.sessionecho.dev").unwrap();

        assert!(db_dir.ends_with("de.flagbit.sessionecho.dev"));
    }
