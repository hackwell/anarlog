...
describe("SyncStatusIndicator", () => {
  ...
  it("distinguishes device approval from first-device setup", async () => {
    ...
    expect(
      screen.getByText(
        "Open Session Echo on a device that already has access, then approve this device.",
      ),
    ).toBeTruthy();
  ...
  it.each([
    ...
    [
      "not_entitled",
      "Session Echo Pro required",
      "Session Echo Pro is required to use cloud sync.",
    ],
    [
      "identity_mismatch",
  ...
  it("shows a retryable error when local sync status cannot be read", async () => {
    ...
    expect(
      screen.getByText(
        "Session Echo couldn't read cloud sync status. Your notes are still available locally.",
      ),
    ).toBeTruthy();
  ...
  it("explains background recovery without implying local notes are blocked", async () => {
    ...
    expect(
      screen.getByText(
        "Session Echo is repairing cloud sync in the background. Your notes remain available locally.",
      ),
    ).toBeTruthy();
    expect(screen.queryByText("Connecting...")).toBeNull();
  });
  ...
  it("shows a non-spinning delayed state after a recovery failure", async () => {
    ...
    expect(
      screen.getByText(
        "Session Echo will keep retrying in the background. Your notes remain available locally.",
      ),
    ).toBeTruthy();
    expect(
      screen
