...
export function CloudApiSection() {
  ...
  const toggleMutation = useMutation({
    ...
    onSuccess: ({ settings, uploaded, backfillFailed }) => {
      ...
      if (backfillFailed) {
        sonnerToast.error(
          t`Cloud API enabled, but existing meetings could not be uploaded. Session Echo will retry.`,
        );
  ...
  return (
    <div className="min-w-0">
      ...
      <p className="text-muted-foreground mt-1 text-xs">
        <Trans>
          Uploads meeting content for remote access while Session Echo is
          closed.
        </Trans>
      </p>
