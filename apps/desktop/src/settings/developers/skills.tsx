...
export function SkillsRow() {
  ...
  const installMutation = useMutation({
    ...
    onSuccess: (statuses) => {
      ...
      sonnerToast.success(
        statuses.length === 1
          ? t`Session Echo skill added to ${statuses[0].displayName}`
          : t`Session Echo skill added to ${statuses.length} agents`,
      );
    },
    onError: (error) => {
      void queryClient.invalidateQueries({ queryKey: SKILL_AGENTS_QUERY_KEY });
    ...
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div className="min-w-0">
        ...
        <p className="text-muted-foreground mt-1 text-xs">
          <Trans>
            Teach coding agents when and how to use the Session Echo CLI and MCP
          </Trans>
        </p>
      </div>
