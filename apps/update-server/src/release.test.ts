...
test("finds the manifest among the release assets", () => {
  assert.equal(
    latestJsonAsset(release(["latest.json", "app.dmg"]))?.name,
    "latest.json",
  );
...
test("keeps the architectures apart", () => {
  assert.equal(
    updaterFor(manifest, "darwin", "x86_64")?.assetName,
    "anarlog-macos-x86_64.app.tar.gz",
  );
  assert.equal(
    updaterFor(manifest, "darwin", "x86_64")?.signature,
    "signature-x86_64",
  );
});
...
test("does not mistake the updater bundle for the disk image", () => {
  assert.equal(
    diskImage(release(["anarlog-macos-aarch64.app.tar.gz"]), "aarch64"),
    null,
  );
});
