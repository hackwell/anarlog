...
// TODO: upstream commit 150c8a1 did not work here. webview_window not found.
...
mod test {
    ...
    fn navigate_from_str() {
        let test_cases = vec![
            (
                "anarlog://sessionecho.flagbit.de/app/new?calendarEventId=123&record=true",
                "/app/new",
                Some(serde_json::json!({ "calendarEventId": "123", "record": "true" })),
            ),
            (
                "sessionecho://sessionecho.flagbit.de/app/new?record=true",
                "/app/new",
