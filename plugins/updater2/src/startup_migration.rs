...
fn legacy_target_app_path(current_app_path: &Path) -> Option<PathBuf> {
    let target_name = match current_app_path.file_name().and_then(|name| name.to_str()) {
        Some("Session Echo.app") | Some("Char.app") => "Session Echo.app",
        Some("Session Echo Nightly.app") | Some("Char Nightly.app") => "Session Echo Nightly.app",
        Some("Session Echo Staging.app") | Some("Char Staging.app") => "Session Echo Staging.app",
        _ => return None,
    };
...
mod tests {
    ...
    fn maps_legacy_bundle_names_to_anarlog_names() {
        let cases = [
            (
                "/Applications/Session Echo.app",
                "/Applications/Session Echo.app",
            ),
            ("/Applications/Char.app", "/Applications/Session Echo.app"),
            (
                "/Applications/Session Echo Nightly.app",
                "/Applications/Session Echo Nightly.app",
            ),
            (
            ...
            (
                "/Applications/Session Echo Staging.app",
                "/Applications/Session Echo Staging.app",
            ),
            (
    ...
    fn relaunch_args_append_skip_flag_and_preserve_other_flags() {
        let args = relaunch_args([
            OsString::from("/Applications/Session Echo Nightly.app/Contents/MacOS/char"),
    ...
    fn rename_command_does_not_open_existing_target_bundle() {
        ...
        let command = build_bundle_rename_command(
            4242,
            Path::new("/Applications/Session Echo Nightly.app"),
            Path::new("/Applications/Session Echo Nightly.app"),
            &relaunch_args,
        );
        let args = command
        ...
        assert!(args[1].contains("if [ -e '/Applications/Session Echo Nightly.app' ]; then"));
    ...
    fn rename_command_reopens_current_bundle_on_failure() {
        ...
        assert!(args[1].contains(
            "open -n '/Applications/Session Echo Nightly.app' --args '--onboarding=123' '--updater2-skip-startup-migration=1'"
        ));
    ...
    fn current_bundle_path_from_executable_uses_bundle_root() {
        let executable = Path::new("/Applications/Session Echo.app/Contents/MacOS/anarlog");

        let bundle = current_app_bundle_path_from_executable(executable).unwrap();

        assert_eq!(bundle, PathBuf::from("/Applications/Session Echo.app"));
    }
    ...
    fn rename_command_relaunches_from_target_bundle() {
        ...
        assert!(args[1].contains(
            "mv -f '/Applications/Session Echo Nightly.app' '/Applications/Session Echo Nightly.app'"
        ));
        assert!(args[1].contains(
            "open -n '/Applications/Session Echo Nightly.app' --args '--onboarding=123' '--updater2-skip-startup-migration=1'"
        ));
    ...
    fn rename_command_relaunches_stable_bundle_with_skip_flag() {
        let relaunch_args = relaunch_args([OsString::from(
            "/Applications/Session Echo.app/Contents/MacOS/char",
        )]);
        let command = build_bundle_rename_command(
            4242,
            Path::new("/Applications/Char.app"),
            Path::new("/Applications/Session Echo.app"),
            &relaunch_args,
        );
        let args = command
            .get_args()
        ...
        assert!(
            args[1].contains("mv -f '/Applications/Char.app' '/Applications/Session Echo.app'")
        );
        assert!(args[1].contains(
            "open -n '/Applications/Session Echo.app' --args '--updater2-skip-startup-migration=1'"
        ));
    }
}
